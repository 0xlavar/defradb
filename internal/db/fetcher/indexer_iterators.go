...
func (iter *inIndexIterator) createIteratorForNextValue() error {
	if iter.isUnique {
		...
		indexIter := iter.fetcher.newPrefixBaseMatchIterator(indexKey, iter.matchers, iter.fetcher.execInfo)
		ordered, reverse := CanBeOrderedByIndex(
			iter.fetcher.ordering,
			iter.fetcher.indexDesc,
			iter.fetcher.mapping,
		)
		if ordered {
			indexIter.Reverse(reverse)
		}
		iter.indexIterator = indexIter
	}
